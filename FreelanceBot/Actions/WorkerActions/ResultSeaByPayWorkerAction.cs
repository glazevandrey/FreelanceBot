...
{
    public class ResultSeaByPayWorkerAction : IBaseAction
    {
        public async Task Start(Update update)
        {
            ...
            {
                list = db.Jobs.Where(m => m.IsDone == true && m.Pay == Convert.ToInt32(pay)).ToList();
                list.AddRange(db.Jobs.Where(m => m.IsDone == true && m.PayMin <= Convert.ToInt32(pay) && m.PayMax >= Convert.ToInt32(pay) && !list.Contains(m)).ToList());
            }
